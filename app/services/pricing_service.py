...
from app.schemas.pricing import (
    HierarchicalPricingRequest,
    HierarchicalPricingResponse,
    SubCategoryValuationLine
)
...
class PricingService:
    """
    Enterprise Circular Economy Scrap & E-Waste Dynamic Valuation Engine.
    Implements multi-tier degradation factors, metallurgical yields (Gold, Silver, Palladium),
    CO2 abatement metrics, logistics haulage deductions, and statutory floor payouts.
    """

    CO2_SAVINGS_PER_KG = {
        "copper": 4.5,     # kg CO2 saved per kg recycled vs primary mining
        "aluminum": 9.0,   # 95% energy reduction over bauxite smelting
        "iron": 1.6,       # Secondary electric arc vs blast furnace
        "e_waste": 14.0,   # High embodied energy recovery
        "battery": 3.8,    # Secondary lead & lithium cell processing
        "plastic": 2.2,    # Recycled polymer vs virgin petroleum resin
        "paper": 1.1       # Cellulose pulp & tree conservation
    }

    @staticmethod
    def calculate_hierarchical_price(
        req: HierarchicalPricingRequest, 
        db: Session
    ) -> HierarchicalPricingResponse:
        ...
        for item in req.items:
            ...
            if subcat:
                ...
                contamination_factor = getattr(subcat, "contamination_factor", 0.10) or 0.10
            ...
            # 1. Calculate Purity, Oxidation, and Contamination Deductions
            ...
            # Oxidation discount: (rust_pct / 100) * rust_factor * oxidation_multiplier
            ...
            effective_purity = max(base_purity * (1.0 - rust_discount - contam_discount), 0.10)
            ...
            # 2. Precious & Strategic Metals Recovery Calculation (g/tonne converted to total grams)
            ...
            # 3. Processing and Segregation Labor Deduction
            if "e_waste" in item.subcategory_code:
                processing_fee += item.weight_kg * 6.0  # Hazardous dismantling & de-soldering
            elif "battery" in item.subcategory_code:
                processing_fee += item.weight_kg * 4.0  # Acid neutralization & casing strip
            else:
                processing_fee += item.weight_kg * 1.5  # Standard sorting and baling

            # 4. CO2 Abatement Metric
            cat_key = "iron"
            ...
            line_items.append(SubCategoryValuationLine(
                subcategory_code=item.subcategory_code,
                subcategory_name=name_en,
                subcategory_name_hi=name_hi,
                subcategory_name_mr=name_mr,
                weight_kg=item.weight_kg,
                spot_rate_per_kg=spot_rate,
                base_purity=base_purity,
                rust_penalty_deduction=round(rust_discount * 100, 1),
                effective_rate_per_kg=effective_rate,
                line_subtotal_inr=line_total,
                estimated_gold_yield_grams=line_gold,
                estimated_silver_yield_grams=line_silver,
                estimated_palladium_yield_grams=line_palladium
            ))

        # 5. Logistics & Haulage Calculation
        if req.pickup_distance_km > 0:
            logistics_fee = round(
                settings.BASE_PICKUP_FEE_INR + (settings.PER_KM_CHARGE_INR * req.pickup_distance_km), 
                2
            )
        else:
            logistics_fee = 0.0

        # 6. Aggregator Margin & Net Valuation Formula
        total_gross_asset = gross_material_val + req.salvage_reusable_parts_value
        margin_amount = round(total_gross_asset * margin_pct, 2)
        
        # Net Formula: (1 - Margin) * Total Asset - (Processing + Logistics)
        net_val = round(((1.0 - margin_pct) * total_gross_asset) - (processing_fee + logistics_fee), 2)
        
        # Statutory Floor Payout Enforcement
        ...
        # 7. Native Hindi & Marathi Vernacular Spoken Scripts
        precious_note_mr = ""
        precious_note_hi = ""
        ...
        audio_mr = (
            f"एकूण {total_weight:.1f} किलो भंगाराचे अंदाजे मूल्य ₹{int(payable_amount)} आहे. "
            f"बाजार चढउतारानुसार हे ₹{int(price_low)} ते ₹{int(price_high)} राहील.{precious_note_mr} "
            f"यामुळे {round(total_co2_saved, 1)} किलो कार्बन उत्सर्जन वाचवले गेले आहे."
        )
        audio_hi = (
            f"कुल {total_weight:.1f} किलो कबाड़ का अनुमानित मूल्य ₹{int(payable_amount)} है। "
            f"बाजार भाव के अनुसार यह ₹{int(price_low)} से ₹{int(price_high)} के बीच रहेगा।{precious_note_hi} "
            f"इससे {round(total_co2_saved, 1)} किलो कार्बन उत्सर्जन की बचत हुई है।"
        )
        ...
        return HierarchicalPricingResponse(
            gross_material_value_inr=round(gross_material_val, 2),
            salvage_parts_value_inr=req.salvage_reusable_parts_value,
            processing_and_segregation_cost_inr=round(processing_fee, 2),
            logistics_and_haulage_cost_inr=logistics_fee,
            aggregator_margin_inr=margin_amount,
            net_valuation_inr=net_val,
            guaranteed_floor_price_inr=settings.MIN_PRICE_FLOOR_INR,
            final_payable_amount_inr=payable_amount,
            estimated_price_range_low_inr=price_low,
            estimated_price_range_high_inr=price_high,
            total_scrap_weight_kg=round(total_weight, 2),
            total_precious_gold_grams=round(total_gold_grams, 4),
            total_precious_silver_grams=round(total_silver_grams, 4),
            total_precious_palladium_grams=round(total_palladium_grams, 4),
            co2_emissions_saved_kg=round(total_co2_saved, 2),
            line_items=line_items,
            audio_summary_hi=audio_hi,
            audio_summary_mr=audio_mr
        )
