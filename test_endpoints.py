"""
End-to-End Enterprise Verification Script for SIH Smart Scrap & E-Waste Backend.
Tests all 5 pillars:
1. Exclusive Hindi & Marathi Voice-to-Command Engine
2. Two-Tier Hierarchical Taxonomy & Precious Metals Recovery
3. CPCB/SPCB Authorized Recycler Locator
4. Offline CPU Vision Classifier
5. Role Authentication & Pickup Transactions
"""
import sys
import io
import json
...
def run_tests():
    ...
    # Marathi Test: "don kilo tamba aani paach kilo lokhand bhav sanga"
    ...
    # Hindi Test: "दस किलो गत्ता और तीन किलो लोहा पिकअप भेजो"
    ...
    pricing_payload = {
        ...
        "salvage_reusable_parts_value": 100.0,
        "pickup_distance_km": 6.0,
    ...
    # Mumbai Dadar Coordinates: 19.0178, 72.8478
    ...
    img = Image.new("RGB", (224, 224), color=(160, 50, 30))  # Rusty reddish-brown iron beam
    buf = io.BytesIO()
    img.save(buf, format="JPEG")
    res_vision = client.post(
        "/api/v1/vision/analyze-image",
        files={"file": ("rusty_iron_beam.jpg", buf.getvalue(), "image/jpeg")}
    )
    ...
    tx_payload = {
        "material_code": "copper_bare_bright",
        "weight_kg": 5.0,
        "pickup_address": "Shop 14, Dharavi Kumbharwada, Mumbai",
        "pickup_latitude": 19.0434,
        "pickup_longitude": 72.8566,
        "voice_input_transcript": "पाच किलो तांबे पिकअप पाठवा"
    }
