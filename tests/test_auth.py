...
def test_user_registration_and_login(client):
    ...
    # 2. Login with valid credentials via JSON
    ...
    # 3. Read profile with Bearer JWT
...
def test_duplicate_registration_fails(client):
    # Try registering already seeded phone number
...
def test_role_buyer_privilege_enforcement(client, dealer_token, buyer_token):
    # Dealer token cannot update commodity spot rates
    res_dealer = client.put(
        "/api/v1/rates/copper_bare_bright?new_spot_rate_per_kg=710.0",
    ...
    # Buyer token successfully updates spot rates
    res_buyer = client.put(
        "/api/v1/rates/copper_bare_bright?new_spot_rate_per_kg=710.0",
