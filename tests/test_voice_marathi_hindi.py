...
def test_marathi_voice_price_inquiry(client):
    # Spoken Marathi: "Two kilos of copper and five kilos of iron, tell me the price"
    phrase = "don kilo tamba aani paach kilo lokhand bhav sanga"
    res = client.post("/api/v1/voice/parse", json={
        "transcript": phrase,
        "language": "mr"
    })
    ...
    # Check detected materials and weights
...
def test_marathi_vernacular_number_parsing(client):
    # Test cardinal sequence: "ardha", "ek", "don", "paach", "daha", "pandhra", "vis", "adhich"
    test_cases = [
        ("ardha kilo tamba bhav sanga", "copper_bare_bright", 0.5),
        ("ek kilo pital dar sanga", "brass_honey", 1.0),
        ("don kilo lokhand mulya sanga", "iron_hms_heavy", 2.0),
        ("paach kilo tambe", "copper_bare_bright", 5.0),
        ("daha kilo raddi gatta", "paper_corrugated_carton", 10.0),
        ("pandhra kilo raddi akhbar", "paper_old_newspaper", 15.0),
        ("vis kilo sariya wajan dakhva", "iron_hms_heavy", 20.0),
        ("adhich kilo pital mulya sanga", "brass_honey", 2.5),
        ("dedh kilo tamba", "copper_bare_bright", 1.5),
    ]
...
def test_hindi_voice_pickup_request_and_numbers(client):
    # Spoken Hindi: "दस किलो गत्ता और तीन किलो लोहा पिकअप भेजो"
...
def test_regional_slang_and_dialect_terms(client):
    # Marathi and Hindi regional vocabulary mappings:
    # Lokhand/Loha, Bhangar/Kabaad, Patra, Botal, Pital/Peetal
    slang_phrases = [
        ("bhangar lohand dha kilo", "iron_hms_heavy"),
        ("chokha tamba paon kilo", "copper_bare_bright"),
        ("patryacha dabba chaar kilo", "iron_light_sheet"),
        ("plastic batlya teen kilo", "plastic_pet_bottles"),
        ("peetal bartan dhai kilo", "brass_honey")
    ]
...
def test_command_intent_detection(client):
    # 1. Price Inquiry: "mulya sanga" / "bhav sanga"
    ...
    # 2. Pickup Request: "pickup pathva"
...
def test_offline_audio_file_decoding(client):
    # Generate valid audio waveform via offline speech synthesis
    sample_wav = VoiceService.synthesize_vernacular_speech("दोन किलो तांबे", SupportedLanguage.MARATHI)
    
    # Upload to offline audio decoding pipeline
    res = client.post(
        "/api/v1/voice/decode-audio?preferred_language=mr",
        files={"file": ("scrap_command_mr.wav", sample_wav, "audio/wav")}
    )
    assert res.status_code == 200
    data = res.json()
